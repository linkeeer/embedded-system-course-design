...
void Form::ledSetSend(int id)
{
    if(buttonGroup2->find(id)!=0){
	int ledsend=id;
	if(((QCheckBox*)buttonGroup2->find(id))->isChecked())ledsend=10+id;
	 if(socket->state()==QSocket::Connected){
	     textLabel10->setText(QString::fromLocal8Bit("设置LED"));
	    QString s=QString::number(ledsend).sprintf("%04d",ledsend);
	    s="1005"+s;
	    printf("1005%04d\n", ledsend);
	    socket->writeBlock( s.latin1(), s.length() );
	    socket->flush(); 
	 }else textLabel10->setText(QString::fromLocal8Bit("网络错误"));
    }
}


void Form::setLatSend( int id )
{
    if(buttonGroup3->find(id)!=0){
	int ledsend=id;
	if(((QCheckBox*)buttonGroup3->find(id))->isChecked())ledsend=100+id;
	 if(socket->state()==QSocket::Connected){
	     textLabel11->setText(QString::fromLocal8Bit(""));
	    QString s=QString::number(ledsend).sprintf("%04d",ledsend);
	    s="1006"+s;
	    printf("1006%04d\n", ledsend);
	    socket->writeBlock( s.latin1(), s.length() );
	    socket->flush(); 
	 }else textLabel11->setText(QString::fromLocal8Bit("网络错误"));
    }

}


void Form::clearAllCheckBox()
{
    for(int i=1;i<=64;++i){
	if(buttonGroup3->find(i)!=0){
	    ((QCheckBox*)buttonGroup3->find(i))->setChecked(false);
	}
    }
    if(socket->state()==QSocket::Connected){
	textLabel11->setText(QString::fromLocal8Bit(""));
